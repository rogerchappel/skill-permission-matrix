# Changelog

All notable changes to this project will be documented in this file.

## Unreleased

- Exclude subject-prefixed natural prohibitions from external-action findings while preserving affirmative actions after statement boundaries.
- Build distributable files during `npm pack` and verify a clean tarball install through both the CLI and library entry point.

## 0.1.0 - 2026-06-29

- Documented the initial skill permission matrix CLI and fixture set.
- Added release readiness coverage for package contents alongside typecheck, tests, build, and smoke verification.
- Published local-first safety and package metadata for release review.
